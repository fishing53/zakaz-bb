...
async function enablePush() {
  ...
  try {
    ...
    await PushNotifications.addListener('registration', async ({ value }) => { await request('/waiter/devices', { method: 'POST', body: JSON.stringify({ token: value, platform: 'android' }) }); });
...
function render(requests:Request[],orders:Order[],alert?:Request){ root.innerHTML=`<header><span>BROOKLYN BOWL</span><strong>${waiter?.name??''}</strong></header><section class="dashboard"><div class="hero"><span>СМЕНА АКТИВНА</span><h1>Ваши<br><em>задачи</em></h1><p>Новые вызовы и заказы появляются автоматически.</p></div><div class="columns"><section><h2>Вызовы</h2>${requests.length?requests.map(r=>`<article class="task ${r.status==='new'?'new':''}"><span>СТОЛ №${r.table_number}</span><h3>${title[r.request_type]??r.request_type}</h3><small>${new Date(r.created_at).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</small>${r.status==='new'?`<button data-id="${r.id}">Принять</button>`:'<b>Принято вами</b>'}</article>`).join(''):'<p class="empty">Нет новых вызовов</p>'}</section><section><h2>Активные заказы</h2>${orders.map(o=>`<article class="order"><span>СТОЛ №${o.table_number}</span><h3>${o.order_number}</h3><b>${o.total} ₽</b></article>`).join('')||'<p class="empty">Нет активных заказов</p>'}</section></div></section>${alert?`<div class="incoming"><span>НОВЫЙ ВЫЗОВ</span><h1>СТОЛ №${alert.table_number}</h1><p>${title[alert.request_type]??alert.request_type}</p><button data-id="${alert.id}">Принять вызов</button></div>`:''}`;root.querySelectorAll<HTMLButtonElement>('[data-id]').forEach(b=>b.onclick=()=>accept(Number(b.dataset.id))); }
