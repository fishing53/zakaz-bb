...
Официальная OpenAPI-схема назначает каждому методу restriction group. Числовые
квоты задаются конкретному API-логину и отображаются в iikoWeb, поэтому их нельзя
считать универсальными для всех ресторанов. BB Kiosk использует более низкие
локальные потолки на скользящем окне 10 минут:

| Группа iiko | Методы BB Kiosk | Локальный потолок |
| --- | --- | ---: |
| `Authorization` | `/api/v2/access_token` | 8 |
| `Data: dictionaries` | организации, terminal groups, order types, скидки | 12 |
| `POS: availability` | доступность terminal group | 6 |
| `Data: menu` | список и снимок внешнего меню | 6 |
| `Orders: preparing` | секции и столы | 6 |
| `Data: stoplists` | полный снимок стоп-листа | 8 |
| `Orders: receiving` | получение статусов заказов | 8 |
| `Organizations: settings` | чтение webhook-настроек | 3 |
| `WebHooks: settings` | изменение webhook-настроек | 1 |
| `Orders: creating` | реальные заказы гостей | 120 |

Ограничения применяются отдельно по группам: исчерпанный лимит настройки webhook
не должен блокировать создание гостевого заказа. Ответ `429` включает backoff
только для соответствующей группы с учётом `Retry-After`.

- webhook — основной источник статусов и стоп-листа;
- общий menu/table/stop-list sync — раз в 10 минут;
- после перезапуска используется сохранённый снимок, первый sync происходит по
  обычному десятиминутному интервалу, поэтому рестарт не создаёт burst;
- отдельный двухминутный polling стоп-листа удалён;
- активные заказы сверяются раз в 2 минуты одним `/order/by_id` с массивом до 30
  ID, а не отдельным запросом на каждый заказ;
- access token кешируется 14 минут, параллельные запросы используют одну задачу
  авторизации;
- изменение webhook выполняется только мастером подключения: один `settings`,
  при необходимости один `update_settings`, без циклических проверок и откатов;
- перезапуск сервера никогда не изменяет webhook-настройки;
- параллельные menu/stop-list/order sync объединяются;
