...
Данные подключения iiko вводятся в веб-админке → «Проверка iiko» → «Настройки подключения iiko». Вручную нужны только `App ID`, `API Login` и `Client Secret`. Сервер через Cloud API получает доступные организации, терминальные группы, внешние меню и типы заказов; администратор выбирает варианты по названию, не работая с UUID. Российский адрес Cloud API, источник заказа, webhook-токен, callback и фильтры событий устанавливаются автоматически.
...
При автоматической настройке в iiko передаётся зашифрованно хранимый `authToken`. Сервер принимает его в заголовке `Authorization` в одном из форматов:
...
### Автоматическая регистрация webhook через Cloud API

Эти запросы выполняет сервер при применении подключения и один раз проверяет после запуска. Администратор не вводит URL или токен вручную.
...
Сохранение считается успешным только после того, как контрольный ответ содержит наш HTTPS-адрес, тот же токен и подписки `TableOrderUpdate`, `TableOrderError` и `StopListUpdate`.
