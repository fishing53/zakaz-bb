...
const iikoChoices = (product: Product) => (product.modifier_groups ?? []).map((group, groupIndex) => group.items.length ? `<section class="option-group option-group--modifiers" data-multiple="${(group.maxQuantity ?? 99) > 1}" data-min-quantity="${group.minQuantity ?? 0}" data-iiko-group="${groupIndex}"><div class="option-group__heading"><h3>${escapeHtml(group.name)}</h3><span>${group.minQuantity ? `Выберите минимум ${group.minQuantity}` : 'По желанию'}</span></div><div class="option-chips">${group.items.map((item) => `<button data-action="set-option" data-iiko-modifier="true" data-product-id="${escapeHtml(item.productId)}" data-value="${escapeHtml(item.name)}" data-price="${item.price}" data-image="${escapeHtml(item.image || '/images/sauce-fallback.webp')}" data-allergens="${escapeHtml(item.allergens ?? '')}" data-max-quantity="${item.maxQuantity ?? group.maxQuantity ?? 20}" class="option-chip${item.defaultQuantity ? ' is-selected' : ''}"><span class="option-chip__check">${icon('check')}</span><span class="option-chip__label">${escapeHtml(item.name)}</span><small>${item.price ? `+${formatPrice(item.price)}` : 'Включено'}</small></button>`).join('')}</div></section>` : '').join('');
