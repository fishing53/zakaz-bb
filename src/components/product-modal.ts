...
const choice = (name: string, values?: string[], multiple = false, price = 0) => values?.length
  ? `<section class="option-group" data-multiple="${multiple}" data-option-group="${name}"><div class="option-group__heading"><h3>${name}</h3>${multiple ? '<span><b data-sauce-count>0</b> выбрано</span>' : ''}</div><div class="option-chips">${values.map((value, index) => `<button data-action="set-option" data-option="${name}" data-value="${escapeHtml(value)}" aria-pressed="${!multiple && index === 0}" class="option-chip${name === 'Соусы' ? ' option-chip--sauce' : ''}${!multiple && index === 0 ? ' is-selected' : ''}"><span class="option-chip__check">${icon('check')}</span><span class="option-chip__label">${escapeHtml(value)}</span>${name === 'Соусы' && price > 0 ? `<small>+${formatPrice(price)}</small>` : ''}</button>`).join('')}</div></section>`
  : '';

const iikoChoices = (product: Product) => (product.modifier_groups ?? []).map((group, groupIndex) => group.items.length ? `<section class="option-group option-group--modifiers" data-multiple="${(group.maxQuantity ?? 99) > 1}" data-min-quantity="${group.minQuantity ?? 0}" data-iiko-group="${groupIndex}"><div class="option-group__heading"><h3>${escapeHtml(group.name)}</h3><span>${group.minQuantity ? `Выберите минимум ${group.minQuantity}` : 'По желанию'}</span></div><div class="option-chips">${group.items.map((item) => `<button data-action="set-option" data-iiko-modifier="true" data-product-id="${escapeHtml(item.productId)}" data-value="${escapeHtml(item.name)}" data-price="${item.price}" data-image="${escapeHtml(item.image || '/images/sauce-fallback.webp')}" data-allergens="${escapeHtml(item.allergens ?? '')}" data-max-quantity="${item.maxQuantity ?? group.maxQuantity ?? 20}" aria-pressed="${Boolean(item.defaultQuantity)}" class="option-chip${item.defaultQuantity ? ' is-selected' : ''}"><span class="option-chip__check">${icon('check')}</span><span class="option-chip__label">${escapeHtml(item.name)}</span>${item.price > 0 ? `<small>+${formatPrice(item.price)}</small>` : ''}</button>`).join('')}</div></section>` : '').join('');
    ...
    <section class="product-modal">
      ...
      <div class="product-modal__body">
        <div class="product-modal__scroll">
          ...
          <p class="product-modal__description">${escapeHtml(product.description || 'О блюде подробнее расскажет официант.')}</p>
          ${spicyLabel(display.spicy) ? `<div class="product-facts product-facts--primary">${spicyLabel(display.spicy)}</div>` : ''}
          <details class="nutrition-details"><summary><span>Состав, КБЖУ и аллергены</span><small>Показать подробности</small>${icon('plus')}</summary><div class="nutrition-details__content"><section class="product-modal__composition"><h3>Состав</h3><p>${escapeHtml(product.composition?.trim() || 'Состав можно уточнить у официанта.')}</p></section><section class="product-modal__allergens"><h3>Аллергены</h3><p data-product-allergens data-base-allergens="${escapeHtml(baseAllergens)}">${allergens ? escapeHtml(allergens) : 'Информацию об аллергенах уточните у официанта.'}</p></section>${product.kbju ? `<section class="nutrition"><header class="nutrition-details__heading"><h3>КБЖУ</h3><small>НА ПОРЦИЮ</small></header><div><span><b>${escapeHtml(product.kbju.calories)}</b><small>ККАЛ</small></span><span><b>${escapeHtml(product.kbju.protein)}</b><small>БЕЛКИ</small></span><span><b>${escapeHtml(product.kbju.fat)}</b><small>ЖИРЫ</small></span><span><b>${escapeHtml(product.kbju.carbs)}</b><small>УГЛЕВОДЫ</small></span></div></section>` : '<p class="nutrition-details__empty">Информацию о пищевой ценности уточните у официанта.</p>'}</div></details>
