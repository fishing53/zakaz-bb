...
export const paymentPage = (lines: CartLine[], productFor: (line: CartLine) => Product | undefined, subtotal: number, discount: number, total: number, comment: string) => {
  ...
  const reviewLines = dishes.map((line) => {
    ...
    const modifiers = (line.modifiers ?? []).map((modifier) => `<article class="review-line review-line--modifier"><div><span>ДОПОЛНЕНИЕ</span><h3>${escapeHtml(modifier.name)} <small>×${modifier.amount * line.quantity}</small></h3><p>К блюду «${escapeHtml(product.name)}»</p></div><strong>${formatPrice(modifier.price * modifier.amount * line.quantity)}</strong></article>`).join('');
    return `<article class="review-line"><div><h3>${escapeHtml(product.name)} <small>×${line.quantity}</small></h3>${details.length ? `<p>${details.join(' · ')}</p>` : ''}</div><strong>${formatPrice((line.customPrice ?? product.price_rub) * line.quantity)}</strong></article>${modifiers}`;
  }).join('');
  const standaloneSauces = lines.filter((line) => line.kind === 'sauce' && !dishes.some((dish) => dish.productId === line.productId));
  const positionCount = lines.reduce((sum, line) => sum + line.quantity + (line.modifiers ?? []).reduce((modifierSum, modifier) => modifierSum + modifier.amount * line.quantity, 0), 0);
  ...
  return `<section class="payment-page payment-confirmation payment-review">
    ...
    <div class="payment-review__layout">
      <section class="payment-review__items"><div class="payment-review__items-head"><h2>Состав заказа</h2><span>${positionCount} поз.</span></div><div class="payment-review__list">${reviewLines}${standaloneSauces.map((line) => `<article class="review-line"><div><h3>${escapeHtml(line.customName ?? 'Соус')} <small>×${line.quantity}</small></h3></div><strong>${formatPrice((line.customPrice ?? 0) * line.quantity)}</strong></article>`).join('')}</div>${comment ? `<div class="payment-review__comment"><span>Комментарий к заказу</span><p>${escapeHtml(comment)}</p></div>` : ''}</section>
