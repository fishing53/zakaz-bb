...
export function welcomePage(banners: Banner[], terminal: TerminalSettings | null) {
  ...
  return `<section class="welcome-home">
    <div class="welcome-banners" aria-label="Баннеры ресторана">
      ${slides}
      <button class="welcome-table-badge" data-action="admin-tap" aria-label="Текущий стол">
        <span>СТОЛ</span><strong>${tableNumber ? `№${escapeHtml(tableNumber)}` : 'НЕ ВЫБРАН'}</strong>
      </button>
