...
export function menuPage(categories: string[], products: Product[], selected: string, search: string, recent: Product[], display: Record<string, ProductDisplaySettings> = {}) {
  return `<section class="menu-page"><header class="menu-toolbar"><nav class="category-nav">${['Все блюда', ...categories].map((category) => `<button data-action="select-category" data-category="${escapeHtml(category)}" class="${selected === category ? 'is-active' : ''}">${escapeHtml(category)}</button>`).join('')}</nav><label class="search-box">${icon('search')}<input data-action="search" value="${escapeHtml(search)}" placeholder="Поиск блюд..." autocomplete="off"/></label></header><section class="menu-content" data-menu-results>${menuResults(products, selected, search, recent, display)}</section></section>`;
}
