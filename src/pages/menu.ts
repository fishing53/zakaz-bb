...
export function menuPage(categories: MenuCategory[], products: Product[], selected: string, search: string, recent: Product[], display: Record<string, ProductDisplaySettings> = {}, ready = true) {
  const tabs = [{ id: 'all', name: 'Все блюда' }, ...categories];
  return `<section class="menu-page"><header class="menu-toolbar"><nav class="category-nav">${tabs.map((category) => `<button data-action="select-category" data-category="${escapeHtml(category.id)}" class="${selected === category.id ? 'is-active' : ''}">${escapeHtml(category.name)}</button>`).join('')}</nav><label class="search-box${search ? ' is-open' : ''}" aria-label="Поиск блюд">${icon('search')}<input data-action="search" value="${escapeHtml(search)}" placeholder="Поиск по всему меню..." autocomplete="off"/><button type="button" class="search-box__close" data-action="close-search" aria-label="Закрыть поиск">${icon('close')}</button></label></header><section class="menu-content" data-menu-results>${menuResults(products, selected, search, recent, display, ready)}</section></section>`;
}
